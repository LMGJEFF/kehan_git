...
def get_pigs():
  ...
  # the correct value.
  soup = BeautifulSoup(html_doc, "html.parser")
  res = [item.string for item in soup.find(text = "three little pigs").findNext("ol").find_all("h2")]
...
def get_story_headings():
  ...
  # the correct value.
  soup = BeautifulSoup(html_doc, "html.parser")
  res = [item.string for item in soup.find_all("h2", text = re.compile("Story"))]
  return json.dumps(res) 
...
def get_houses():
  ...
  for item in soup.find("table").find_all("tr"):
  	res.append(tuple([x.string for x in item.find_all("td")]))
...
def get_links():
  ...
  # the correct value.
  soup = BeautifulSoup(html_doc, "html.parser")
  res = [item.get("href") for item in soup.find_all("a")]
  return json.dumps(res)
...
def treasure_hunting():
  # Your code here: make sure to modify the return statement to pass back
  # the correct value.
  soup = BeautifulSoup(urlopen("http://www.example.com").read(), "html.parser")
  link = soup.find("a").get("href")
  soup = BeautifulSoup(urlopen(link).read(), "html.parser")
  res = soup.find(text = "Test IDN top-level domains").findNext("table").find_all("tr")[3].td.string
  return str(res)
